...
export function ExpenseTable({expenses, showOnly = false, range, onRangeChange}: Props) {
    const {t, locale} = useI18n()
    const queryClient = useQueryClient()
    const [openEdit, setOpenEdit] = useState<boolean>(false)
    const [addExpense, setAddExpense] = useState<boolean>(false)
    const [addExpenseType, setAddExpenseType] = useState<'other' | 'inventory_purchase'>('other')
    ...
    const expenseTypeLabel = (expense: Expense) => {
        if (expense.type === 'inventory_purchase') return locale === 'en' ? 'Ingredient' : locale === 'zh-TW' ? '原料' : 'Nguyên liệu'
        const labels = locale === 'en'
            ? {utilities: 'Utilities', rent: 'Rent', transport: 'Transport', maintenance: 'Maintenance', salary: 'Salary', other: 'Other'}
            : locale === 'zh-TW'
                ? {utilities: '水電', rent: '租金', transport: '運輸', maintenance: '維修', salary: '薪資', other: '其他'}
                : {utilities: 'Điện nước', rent: 'Thuê mặt bằng', transport: 'Vận chuyển', maintenance: 'Sửa chữa', salary: 'Nhân sự', other: 'Khác'}
    ...
    useEffect(() => {
        ...
        if (!element) return
        const resize = () => {
            const toolbarHeight = element.querySelector<HTMLElement>('[data-expense-toolbar]')?.getBoundingClientRect().height ?? 148
            const tableHeaderHeight = element.querySelector<HTMLElement>('thead')?.getBoundingClientRect().height ?? 48
            ...
            const calculatedPageSize = Math.floor((element.clientHeight - toolbarHeight - tableHeaderHeight - 8) / Math.max(rowHeight, 52))
        ...
        <div ref={tableRef} className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div data-expense-toolbar className="mb-2 flex flex-col gap-2">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Input value={totalPrice.toLocaleString()} disabled className="w-48" />
                    {range && <span className="whitespace-nowrap text-xs text-muted-foreground">{locale === 'en' ? 'Period:' : locale === 'zh-TW' ? '期間：' : 'Kỳ:'} {formatDateTime(range.from)} → {formatDateTime(range.to)}</span>}
                    <Input
                        ...
                        className="w-48"
                    />
                    {search && <Button variant="ghost" size="sm" onClick={() => { setSearch(''); setPage(1) }}>{t('delete')}</Button>}
                    {!showOnly && <Button className='shrink-0 bg-primary text-primary-foreground hover:bg-primary/90' onClick={() => { setAddExpenseType('other'); setAddExpense(true) }}>{t('addExpense')}</Button>}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    {range && <>
                        <span className="text-xs text-muted-foreground">{locale === 'en' ? 'From' : locale === 'zh-TW' ? '從' : 'Từ'}</span>
                        <Input disabled={!onRangeChange} ref={fromInputRef} type="datetime-local" className="h-8 w-48 px-2 text-xs" defaultValue={dateInputValue(range.from)} key={`from-${range.from}`} />
                        <span className="text-xs text-muted-foreground">{locale === 'en' ? 'to' : locale === 'zh-TW' ? '至' : 'đến'}</span>
                        <Input disabled={!onRangeChange} ref={toInputRef} type="datetime-local" className="h-8 w-48 px-2 text-xs" defaultValue={dateInputValue(range.to || currentTime)} key={`to-${range.to || 'current'}`} />
                        <Button disabled={!onRangeChange} size="sm" onClick={() => onRangeChange?.({ from: dateInputToIso(fromInputRef.current?.value || ''), to: dateInputToIso(toInputRef.current?.value || '') })}>
                            {locale === 'en' ? 'Search' : locale === 'zh-TW' ? '搜尋' : 'Tìm'}
                        </Button>
                    </>}
                    <div className="ml-auto flex items-center gap-2">
                        <span className="whitespace-nowrap text-sm text-gray-500">{filteredOrders.length} {t('expenseCount')} • {t('page')} {currentPage}/{totalPages || 1}</span>
            ...
            <Table>
                <TableHeader className="sticky top-0  z-10">
                    <TableRow>
                        ...
                        <TableHead className="whitespace-nowrap">{locale === 'en' ? 'Time' : locale === 'zh-TW' ? '時間' : 'Thời gian'}</TableHead>
                    ...
                    {paginatedOrders.map((exp) => {
                        ...
                        return (
                            <TableRow key={exp._id}>
                                ...
                                <TableCell>{(exp.unitPrice ?? exp.price).toLocaleString()}</TableCell>
            ...
            {addExpense && <AddExpenseDialog initialEntryType={addExpenseType} open={addExpense} onClose={() => setAddExpense(false)}/>}
        </div>
