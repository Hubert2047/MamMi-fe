...
export function ImageUploadField({ label, hint, chooseLabel, removeLabel, value, pendingFile, onChange, onFileChange, onError, className }: Props) {
  ...
  return <div className={`min-w-0 space-y-2 ${className || ''}`}>
    ...
    <div className="flex h-10 min-w-0 items-center gap-2 overflow-hidden"><input ref={inputRef} type="file" accept="image/*" onChange={(event) => { selectImage(event.target.files?.[0]); event.currentTarget.value = '' }} className="sr-only" /><Button type="button" className="h-10 min-w-0 flex-1 truncate" variant="outline" onClick={() => inputRef.current?.click()}>{chooseLabel}</Button>{preview && <><img src={preview} alt="" className="h-10 w-10 shrink-0 rounded object-cover" /><Button type="button" size="icon" className="h-10 w-10 shrink-0" variant="destructive" title={removeLabel} aria-label={removeLabel} onClick={() => { setPreview(undefined); setPreviewObjectUrl(null); onFileChange(null); onChange('') }}><Trash2 className="h-4 w-4" /></Button></>}</div>
  </div>
}
