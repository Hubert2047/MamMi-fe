...
export default function StoreAddonsPanel() {
  ...
  const filteredStoreAddons = useMemo(() => {
    ...
    if (!normalizedSearch) return storeAddons;
    return storeAddons.filter((addon) =>
      normalizeSearchText(name(addon)).includes(normalizedSearch),
    );
  }, [search, storeAddons, locale]);
  const totalPages = Math.max(
    1,
    Math.ceil(filteredStoreAddons.length / pageSize),
  );
  const currentPage = Math.min(page, totalPages);
  const paginated = useMemo(
    () =>
      filteredStoreAddons.slice(
        (currentPage - 1) * pageSize,
        currentPage * pageSize,
      ),
    [currentPage, filteredStoreAddons, pageSize],
  );
  ...
  return (
    ...
    >
      ...
      <Card
        ...
        className="flex min-h-0 flex-1 flex-col gap-0 border border-slate-300 shadow-sm"
      >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">
                {t("total")}: {filteredStoreAddons.length}
              </span>
              <Button
                size="sm"
                variant="outline"
