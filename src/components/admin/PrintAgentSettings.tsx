...
export default function PrintAgentSettings() {
  ...
  return <div className='space-y-6'>
    ...
    <Card><CardHeader><CardTitle>{t('printAgentRoutingTitle')}</CardTitle><CardDescription>{t('printAgentRoutingDescription')}</CardDescription></CardHeader><CardContent className='space-y-4'><div className='grid gap-4 md:grid-cols-3'>{routeSelect(t('printAgentKitchenRoute'), 'kitchenPrinterId')}{routeSelect(t('printAgentReceiptRoute'), 'receiptPrinterId')}{routeSelect(t('printAgentFapiaoRoute'), 'fapiaoPrinterId')}</div><Button disabled={saveRouting.isPending} onClick={saveRoute}>{saveRouting.isPending ? t('saving') : t('save')}</Button></CardContent></Card>
    <Card><CardHeader><CardTitle>{t('printAgentList')}</CardTitle></CardHeader><CardContent className='space-y-4'>{query.isLoading ? <p className='text-sm text-muted-foreground'>{t('printAgentLoading')}</p> : agents.length === 0 ? <p className='text-sm text-muted-foreground'>{t('printAgentEmpty')}</p> : agents.map((agent) => {
      const form = formFor(agent._id)
      return <div key={agent._id} className='space-y-4 rounded-lg border p-4'>
