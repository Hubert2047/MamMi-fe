...
type PrinterForm = { name: string; windowsPrinterName: string; profile: Printer['profile']; printerDpi: number; labelWidthMm: number; labelHeightMm: number; labelGapMm: number }
const printerInitial: PrinterForm = { name: '', windowsPrinterName: '', profile: 'kitchen-label-tspl', printerDpi: 203, labelWidthMm: 58, labelHeightMm: 40, labelGapMm: 2 }
...
export default function PrintAgentSettings() {
  ...
  const [printerEdit, setPrinterEdit] = useState<{ agentId: string; printerId: string; data: PrinterForm } | null>(null)
  ...
  const [createPrinterForm, setCreatePrinterForm] = useState<PrinterForm>(printerInitial)
  ...
  useEffect(() => {
    ...
    root.querySelectorAll<HTMLElement>('[data-slot="card"]').forEach((card) => {
      ...
      if (text.includes(t('printAgentRoutingTitle'))) {
        ...
        card.querySelectorAll<HTMLElement>('[data-slot="select-trigger"]').forEach((trigger) => {
          trigger.style.width = 'fit-content'
          trigger.style.minWidth = '12rem'
        })
        const routeGrid = card.querySelector<HTMLElement>('[data-slot="card-content"] > div')
        if (routeGrid) {
      ...
      if (text.includes(t('printAgentAgentSection'))) card.style.display = activeTab === 'agents' ? '' : 'none'
      if (text.includes(t('printAgentCreateAgent'))) card.style.display = 'none'
      if (text.includes(t('printAgentAgentSection'))) {
        const title = card.querySelector<HTMLElement>('[data-slot="card-title"]')
        if (title) title.style.display = 'none'
        card.querySelectorAll<HTMLElement>('[class*="bg-muted/40"]').forEach((form) => { form.style.display = 'none' })
        const addButton = document.getElementById('print-agent-add-printer-button')
        const agentPanel = card.querySelector<HTMLElement>('.rounded-lg.border-2')
        const actions = agentPanel?.querySelector<HTMLElement>('.flex.flex-wrap.items-center.justify-between > .flex.gap-2')
        if (addButton && actions && addButton.parentElement !== actions) actions.prepend(addButton)
      }
    })
  }, [activeTab, t])
  ...
  const create = useMutation({ mutationFn: createPrinter, onSuccess: () => { invalidate(); setIsCreatePrinterOpen(false); setCreatePrinterForm(printerInitial) } })
  ...
  const savePrinter = useMutation({ mutationFn: (input: { agentId: string; printerId: string; data: PrinterForm }) => updatePrinter(input), onSuccess: () => { setPrinterEdit(null); invalidate() } })
  ...
  return <div ref={settingsRef} className='space-y-3'>
    <div className='flex flex-wrap gap-2 border-b pb-2'><Button size='sm' variant={activeTab === 'agents' ? 'default' : 'outline'} onClick={() => setActiveTab('agents')}>Print Agent</Button><Button size='sm' variant={activeTab === 'routing' ? 'default' : 'outline'} onClick={() => setActiveTab('routing')}>Print Routing</Button></div>
    {activeTab === 'agents' && <div className='hidden'><Button id='print-agent-add-printer-button' size='sm' onClick={() => { setCreatePrinterAgentId(agents[0]?._id || ''); setIsCreatePrinterOpen(true) }}>{t('printAgentAddPrinter')}</Button></div>}
    <Dialog open={isCreatePrinterOpen} onOpenChange={setIsCreatePrinterOpen}><DialogContent className='max-w-2xl'><DialogHeader><DialogTitle>{t('printAgentAddPrinter')}</DialogTitle></DialogHeader><div className='grid gap-3 sm:grid-cols-2'><div className='space-y-1 sm:col-span-2'><Label>{t('printAgentAgentSection')}</Label><Select value={createPrinterAgentId} onValueChange={setCreatePrinterAgentId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{agents.map((agent) => <SelectItem key={agent._id} value={agent._id}>{agent.name}</SelectItem>)}</SelectContent></Select></div><div className='space-y-1'><Label>{t('printAgentName')}</Label><Input value={createPrinterForm.name} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, name: e.target.value })} /></div><div className='space-y-1'><Label>{t('printAgentWindowsName')}</Label><Input value={createPrinterForm.windowsPrinterName} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, windowsPrinterName: e.target.value })} /></div><div className='space-y-1'><Label>{t('printAgentProfile')}</Label><Select value={createPrinterForm.profile} onValueChange={(value) => setCreatePrinterForm({ ...createPrinterForm, profile: value as PrinterForm['profile'] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value='kitchen-label-tspl'>{t('printAgentKitchenProfile')}</SelectItem><SelectItem value='receipt-escpos'>{t('printAgentReceiptProfile')}</SelectItem></SelectContent></Select></div><div className='space-y-1'><Label>{t('printAgentDpi')}</Label><Input type='number' value={createPrinterForm.printerDpi} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, printerDpi: Number(e.target.value) })} /></div><div className='space-y-1'><Label>{t('printAgentWidth')}</Label><Input type='number' value={createPrinterForm.labelWidthMm} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, labelWidthMm: Number(e.target.value) })} /></div><div className='space-y-1'><Label>{t('printAgentHeight')}</Label><Input type='number' value={createPrinterForm.labelHeightMm} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, labelHeightMm: Number(e.target.value) })} /></div><div className='space-y-1'><Label>{t('printAgentGap')}</Label><Input type='number' value={createPrinterForm.labelGapMm} onChange={(e) => setCreatePrinterForm({ ...createPrinterForm, labelGapMm: Number(e.target.value) })} /></div><div className='flex items-end sm:col-span-2'><Button disabled={create.isPending || !createPrinterAgentId || !createPrinterForm.name.trim() || !createPrinterForm.windowsPrinterName.trim()} onClick={() => create.mutate({ agentId: createPrinterAgentId, data: createPrinterForm })}>{create.isPending ? t('saving') : t('printAgentAddPrinter')}</Button></div></div></DialogContent></Dialog>
    <Dialog open={isCreateAgentOpen} onOpenChange={setIsCreateAgentOpen}><DialogContent className='max-w-lg'><DialogHeader><DialogTitle>{t('printAgentCreateAgent')}</DialogTitle></DialogHeader><div className='space-y-3'><Label>{t('printAgentAgentName')}</Label><Input value={agentForm.name} placeholder={t('printAgentAgentNamePlaceholder')} onChange={(event) => setAgentForm({ name: event.target.value })} /><Button disabled={createAgent.isPending || !agentForm.name.trim()} onClick={() => createAgent.mutate(agentForm)}>{createAgent.isPending ? t('saving') : t('printAgentCreateAgent')}</Button></div></DialogContent></Dialog>
    ...
    <Dialog open={Boolean(printerEdit)} onOpenChange={(open) => { if (!open) setPrinterEdit(null) }}><DialogContent className='max-w-lg'><DialogHeader><DialogTitle>{t('printAgentEditPrinter')}</DialogTitle></DialogHeader>{printerEdit && <div className='grid gap-3 sm:grid-cols-2'><div className='space-y-1'><Label>{t('printAgentName')}</Label><Input value={printerEdit.data.name} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, name: event.target.value } })} /></div><div className='space-y-1'><Label>{t('printAgentWindowsName')}</Label><Input value={printerEdit.data.windowsPrinterName} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, windowsPrinterName: event.target.value } })} /></div><div className='space-y-1'><Label>{t('printAgentProfile')}</Label><Select value={printerEdit.data.profile} onValueChange={(value) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, profile: value as typeof printerEdit.data.profile } })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value='kitchen-label-tspl'>{t('printAgentKitchenProfile')}</SelectItem><SelectItem value='receipt-escpos'>{t('printAgentReceiptProfile')}</SelectItem></SelectContent></Select></div><div className='space-y-1'><Label>{t('printAgentDpi')}</Label><Input type='number' value={printerEdit.data.printerDpi} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, printerDpi: Number(event.target.value) } })} /></div><div className='space-y-1'><Label>{t('printAgentWidth')}</Label><Input type='number' value={printerEdit.data.labelWidthMm} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, labelWidthMm: Number(event.target.value) } })} /></div><div className='space-y-1'><Label>{t('printAgentHeight')}</Label><Input type='number' value={printerEdit.data.labelHeightMm} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, labelHeightMm: Number(event.target.value) } })} /></div><div className='space-y-1'><Label>{t('printAgentGap')}</Label><Input type='number' value={printerEdit.data.labelGapMm} onChange={(event) => setPrinterEdit({ ...printerEdit, data: { ...printerEdit.data, labelGapMm: Number(event.target.value) } })} /></div><div className='flex items-end'><Button className='w-full' disabled={savePrinter.isPending || !printerEdit.data.name.trim() || !printerEdit.data.windowsPrinterName.trim()} onClick={() => savePrinter.mutate(printerEdit)}>{t('printAgentSavePrinter')}</Button></div></div>}</DialogContent></Dialog>
    ...
    <Card><CardHeader><CardTitle>{t('printAgentAgentSection')}</CardTitle></CardHeader><CardContent className='space-y-4'>{query.isLoading ? <p className='text-sm text-muted-foreground'>{t('printAgentLoading')}</p> : agents.length === 0 ? <p className='text-sm text-muted-foreground'>{t('printAgentEmpty')}</p> : agents.map((agent) => {
      ...
      return <div key={agent._id} className='space-y-3'>
