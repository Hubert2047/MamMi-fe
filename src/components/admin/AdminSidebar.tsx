...
import { BarChart3, Package, Tags, Percent, Users, ReceiptText, WalletCards, ClipboardList, CalendarCheck, Settings, LogOut, Store } from 'lucide-react'
...
export default function AdminSidebar() {
  ...
  return <aside className="flex h-svh w-72 shrink-0 flex-col border-r bg-card">
    ...
    <div className="border-t p-3"><Button variant="ghost" className="w-full justify-start gap-3 text-muted-foreground" onClick={logout}><LogOut className="size-4" />{t('logout')}</Button><Button variant="outline" className="mt-2 w-full" onClick={() => router.push('/pos')}>{t('backToPos')}</Button></div>
  </aside>
}
