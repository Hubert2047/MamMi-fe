...
export default function AdminSidebar() {
  ...
  const { stores, activeStore, activeStoreId, setActiveStoreId, isLoading } =
    useStoreContext();
  ...
  const settingsLink = (
    <Link href="/admin/settings" className={linkClass("/admin/settings")}>
      ...
      {({ vi: "Cài đặt", en: "Settings", "zh-TW": "設定" } as const)[locale]}
    </Link>
  );
      ...
      <div className="flex items-center gap-2 border-b px-3 py-2">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-xs text-muted-foreground">
            {user?.role}
            {!isSuperAdmin && activeStore?.name ? ` · ${activeStore.name}` : ""}
          </div>
        </div>
      </div>
      {stores.length > 1 && (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger className="min-w-0 flex-1">
                <SelectValue placeholder={activeStore?.name || t("store")} />
              </SelectTrigger>
              <SelectContent>
                {stores.map((store) => (
        ...
        {isSuperAdmin ? (
          <>
            <section>
              ...
              <Link
                href="/admin/overview"
                className={linkClass("/admin/overview")}
              >
