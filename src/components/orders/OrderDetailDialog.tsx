        ...
        <DialogContent className='left-0 top-0 flex h-dvh max-h-dvh w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 rounded-none p-0 sm:max-w-none'>
            ...
            <div className='min-h-0 flex-1 overflow-y-auto p-6'><div className='grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]'>
                <section className='space-y-3'>{order.items.map((item, index) => {
                    ...
                    return <div key={`${item.id}-${index}`} className='rounded-lg border p-4 font-normal'><div className='flex items-start justify-between gap-3'><div><div className='font-semibold'>{localizedName}</div>{localizedVariant && <div className='text-sm text-muted-foreground'>{t('variant')}: {localizedVariant}</div>}<div className='text-sm text-muted-foreground'>{t('quantity')}: {item.quantity} × {money(item.basePrice)}</div></div><div className='text-right font-normal'>{money(Math.max(0, gross - discount))}</div></div>{item.addonDisplayMode !== 'merged' && item.addons.length > 0 && <div className='mt-2 text-sm text-muted-foreground !font-light' style={{ fontWeight: 300 }}>{t('addons')}: {item.addons.map((addon) => `${localizedAddonName(addon)} (${money(addon.priceExtra)})`).join(', ')}</div>}{item.optionSelections?.length ? <div className='mt-1 text-sm !font-light'><span>{t('optionsSelected')}:</span> {item.optionSelections.map(localizedOptionName).join(', ')}</div> : null}{item.noteOptions.length > 0 && <div className='mt-1 text-sm text-muted-foreground !font-light'><span>{t('noAddons')}:</span> {item.noteOptions.map(localizedNoteName).join(', ')}</div>}{item.note && <div className='mt-1 text-sm text-muted-foreground'>{t('note')}: {item.note}</div>}{discount > 0 && <div className='mt-2 text-sm text-primary'>{t('discount')}: -{money(discount)}</div>}</div>
                })}</section>
