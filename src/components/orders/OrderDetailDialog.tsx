...
const targetKey = (target: "order" | "product" | "addon" | "line") =>
  ({
    order: "targetOrder",
...
export default function OrderDetailDialog({ order, onClose }: Props) {
  ...
  const itemGross = (item: OrderItem) =>
    ...
    item.addons.reduce(
      (sum, addon) => sum + addon.priceExtra * addon.amount * item.quantity,
      0,
    );
  const allocationFor = (
      ...
      <DialogContent className="left-0 top-0 flex h-dvh max-h-dvh w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 rounded-none p-0 sm:max-w-none">
        <DialogHeader className="shrink-0 border-b px-6 py-4 pr-14">
          <DialogTitle className="text-xl">
            {t("orderDetailTitle")} #{order.number}
          </DialogTitle>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
              ...
              {order.items.map((item, index) => {
                ...
                const discount = (order.appliedPromotions || []).reduce(
                  (sum, promotion) => {
                    ...
                    return (
                      sum +
                      (allocation?.productDiscountAmount || 0) +
                      (allocation?.addonDiscounts || []).reduce(
                        (addonSum, entry) => addonSum + entry.discountAmount,
                        0,
                      )
                    );
                  },
                ...
                const localizedAddonName = (
                  addon: OrderItem["addons"][number],
                ) =>
                  catalogItem?.addons.find(
                    (candidate) => candidate._id === addon.id,
                  )?.name || addon.name;
                const findLocalizedOption = (
                  options: typeof catalogItem extends undefined
                    ? never[]
                    : NonNullable<typeof catalogItem>["variants"],
                  value: string,
                ) =>
                  options.find(
                    (option) =>
                      option.id === value ||
                      Object.values(option.names || {}).some(
                        (name) => name === value,
                      ),
                  )?.names?.[locale] || value;
                const localizedVariant = item.variant
                  ? findLocalizedOption(
                ...
                const localizedOptionName = (
                  selection: NonNullable<OrderItem["optionSelections"]>[number],
                ) =>
                  findLocalizedOption(
                    catalogItem?.optionGroups.flatMap(
                      (group) => group.options,
                    ) || [],
                    selection.optionId || selection.name || "",
                  );
                const localizedNoteName = (note: string) =>
                  findLocalizedOption(catalogItem?.noteOptions || [], note);
                return (
                  <div
                    key={`${item.id}-${index}`}
                    className="rounded-lg border p-4 font-normal"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="font-semibold">{localizedName}</div>
                        {localizedVariant && (
                          <div className="text-sm text-muted-foreground">
                            {t("variant")}: {localizedVariant}
                          </div>
                        )}
                        <div className="text-sm text-muted-foreground">
                      ...
                      item.addons.length > 0 && (
                        ...
                        >
                          ...
                          {item.addons
                            .map(
                              (addon) =>
                                `${localizedAddonName(addon)} (${money(addon.priceExtra)})`,
                            )
                            .join(", ")}
                        </div>
                      )}
                    ...
                    {item.noteOptions.length > 0 && (
                      <div className="mt-1 text-sm text-muted-foreground !font-light">
                        <span>{t("noAddons")}:</span>{" "}
                        {item.noteOptions.map(localizedNoteName).join(", ")}
                      </div>
                    )}
                    {item.note && (
            ...
            <aside className="space-y-5">
              <section className="rounded-lg border p-4">
                <h3 className="mb-3 font-semibold">{t("promotionDetails")}</h3>
                {order.appliedPromotions?.length ? (
                  <div className="space-y-3">
                    {order.appliedPromotions.map((promotion) => (
                      <details
                        key={`${promotion.promotionId}-${promotion.promotionVersion}`}
              ...
              <section className="rounded-lg border p-4 text-sm">
                ...
                <div className="mt-2 flex justify-between">
                  ...
                  <span className="text-primary">-{money(totalDiscount)}</span>
                </div>
                ...
                <div className="mt-3 border-t pt-3 text-muted-foreground">
                  {t("paymentMethod")}: {t(order.paymentMethod)}
                </div>
                {order.customer && (
        ...
        <div className="shrink-0 border-t bg-muted/30 p-4 text-right">
          <Button className="min-h-11 px-6 text-base" onClick={onClose}>
            {t("close")}
          </Button>
        </div>
      </DialogContent>
