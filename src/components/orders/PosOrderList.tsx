      ...
      <div className="flex min-h-0 flex-1 flex-col space-y-2 overflow-y-auto pr-2 [scrollbar-gutter:stable]">
        {items.map((item, index) => {
          const discount = appliedPromotions
            .filter((promotion) => !promotion.targets?.includes("order"))
            ...
            .reduce(
              (total, allocation) =>
                ...
                allocation.addonDiscounts.reduce(
                  (addonTotal, addon) => addonTotal + addon.discountAmount,
                  0,
                ),
              0,
            );
          const original =
            ...
            item.addons.reduce(
              (acc, addon) =>
                acc + addon.amount * addon.priceExtra * item.quantity,
              0,
            );
          return (
            <Button
              key={item.id + "-" + index}
              variant={
                currentOrderItem.itemId === item.itemId ? "default" : "outline"
              }
