...
export function OrderTable({
  ...
  const [orderAlertEnabled, setOrderAlertEnabled] = useState(
    () =>
      ...
      window.localStorage.getItem("order-alert-enabled") !== "false",
  );
  ...
  const cancelOrderMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient
        .invalidateQueries({ queryKey: queryKeys.orders(range) })
        .then();
      toast.success(t("cancelSuccess"));
    },
    onError: (error: unknown) => {
      const responseData = (
        error as AxiosError<{ code?: string; message?: string }>
      ).response?.data;
      const message =
        responseData?.code === "FINANCIAL_PERIOD_CLOSED"
  ...
  const openCustomerEditor = (order: BaseOrder) => {
    ...
    setPickupDraft(
      order.pickupAt
        ? taipeiInputValue(new Date(order.pickupAt))
  ...
  const saveCustomer = () => {
    ...
    updateCustomerMutation.mutate({
      ...
      pickupAt: pickupDraft
        ? new Date(`${pickupDraft}:00+08:00`).toISOString()
        : undefined,
    });
  };
  ...
  const openPaymentEditor = (order: BaseOrder) => {
    if (
      order.status !== "paid" ||
      !["dine_in", "takeaway"].includes(order.type) ||
      order.source === "uber" ||
  ...
  const savePaymentMethod = () => {
    ...
    updatePaymentMutation.mutate({
      id: paymentOrder._id,
      data: { paymentMethod, version: paymentOrder.version },
    });
  };
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    const matchesSearch =
      !search || o.number.toString().includes(search.trim());
  ...
  useEffect(() => {
    ...
    const resize = () => {
      const toolbarHeight =
        element
          .querySelector<HTMLElement>("[data-order-toolbar]")
  ...
  return (
    <>
      ...
      >
        ...
        >
            ...
            <div data-order-toolbar className="mb-2 flex flex-col gap-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button
                  ...
                  size="icon"
                  aria-label={
                    orderAlertEnabled ? t("orderAlertOff") : t("orderAlertOn")
                  }
                  title={
                    orderAlertEnabled ? t("orderAlertOff") : t("orderAlertOn")
                  }
                  onClick={() =>
                    setOrderAlertEnabled((enabled) => {
                      const next = !enabled;
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <span className="whitespace-nowrap text-sm text-gray-500">
                    {filteredOrders.length} {t("ordersCount")} • {t("page")}{" "}
                ...
                <TableBody>
                  ...
                  {paginatedOrders.map((order) => {
                    const isCanceling =
                      ...
                      cancelOrderMutation.variables?.id === order._id;
                    return (
                      <TableRow key={order._id} className="h-14">
                        ...
                        <TableCell>
                          {t(orderStatusMessageKeys[order.status])}
                        </TableCell>
                        <TableCell>
                          {t(orderSourceMessageKeys[order.source || "pos"])}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {order.status === "pending"
                            ? t("pendingPayment")
                            : t(paymentMethodMessageKeys[order.paymentMethod])}
                        </TableCell>
                        <TableCell>
                          {order.createdAt
          ...
          {focusOrder && openPrintOptions && (
            <PrintOptions
              order={focusOrder}
              open={openPrintOptions}
              onClose={() => setOpenPrintOptions(false)}
            />
          )}
          <Dialog
            open={Boolean(customerOrder)}
            onOpenChange={(isOpen) => {
              if (!isOpen) setCustomerOrder(null);
            }}
          >
            <DialogContent className="top-4 translate-y-0 sm:max-w-md">
              ...
              <div className="grid gap-3">
                <div className="grid gap-1">
                  <Label htmlFor="order-customer-name">{t("customer")}</Label>
                  <Input
                ...
                <div className="grid gap-1">
                  <Label htmlFor="order-customer-phone">{t("phone")}</Label>
                  <Input
                ...
                <div className="grid gap-1">
                  <Label htmlFor="order-pickup-at">{t("pickupTime")}</Label>
                  <Input
                    id="order-pickup-at"
                    type="datetime-local"
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => {
          if (!isOpen) setPaymentOrder(null);
        }}
      >
        <DialogContent className="top-4 translate-y-0 sm:max-w-md">
          <DialogHeader>
          ...
          <Select
            value={paymentMethod}
            onValueChange={(value) =>
              setPaymentMethod(value as "cash" | "bank" | "linepay")
            }
          >
            <SelectTrigger>
              <SelectValue />
      ...
      {(isOrderLoading ||
        cancelOrderMutation.isPending ||
        updatePaymentMutation.isPending) && <Loading />}
      <OrderDetailDialog
        order={detailOrder}
        onClose={() => {
