...
export default function LoginPage() {
  ...
  const [loginStores, setLoginStores] = useState<
  ...
  const handleSubmit = async (event: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!selectingStore) {
        ...
        if (role !== "SuperAdmin" && !stores[0]?._id)
          ...
          <CardContent>
            ...
            >
              ...
              <Button
                type="submit"
                disabled={loading}
                className="mt-1 w-full h-10 bg-primary hover:bg-primary/90 font-semibold tracking-wide gap-2 transition-colors"
              >
