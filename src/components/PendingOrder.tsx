  ...
  return (
    <div className="flex flex-col pt-2 px-4 pb-4 flex-1 gap-3">
      ...
      <div className="flex items-center space-x-2 pt-6">
        <Label
          htmlFor="name"
          className="w-32 shrink-0 whitespace-nowrap text-start"
        >
          {t("customerName")}
        </Label>
        <Input
      ...
      <div className="flex items-center space-x-2">
        <Label
          htmlFor="pickup-at-pending"
          className="w-32 shrink-0 whitespace-nowrap text-start"
        >
          {t("pickupTime")}
        </Label>
        <Input
          id="pickup-at-pending"
          type="datetime-local"
          className="w-50"
          value={
            currentOrder.pickupAt
              ? taipeiInputValue(new Date(currentOrder.pickupAt))
              : ""
          }
          ...
          onChange={(event) =>
            setCurrentOrder((prev) => ({
              ...
              pickupAt: event.target.value
                ? new Date(`${event.target.value}:00+08:00`).toISOString()
                : undefined,
            }))
          }
        />
