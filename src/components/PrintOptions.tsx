...
export default function PrintOptions({ order, open, onClose }: Props) {
  ...
  return (
    <div
      className="absolute inset-0 z-20 flex items-start justify-center bg-black/35 p-4 pt-10"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="w-full max-w-sm rounded-xl border bg-background p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h2 id="print-options-title" className="text-lg font-semibold">
              In Bếp
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              In từng sản phẩm cho khu vực bếp.
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={handlePrint}
          >
            {isPrinting ? "Đang gửi..." : "Bắt đầu in"}
          </Button>
        </div>
      </div>
    </div>
