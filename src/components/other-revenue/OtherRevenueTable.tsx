...
import {deleteRevenue, type IUpdateRevenue, type Revenue, type RevenueRange} from "@/api/other-revenue.ts";
...
export function OtherRevenueTable({revenues, showOnly = false, range, onRangeChange}: Props) {
    ...
    useEffect(() => {
        ...
        if (!element) return
        const resize = () => {
            const toolbarHeight = element.querySelector<HTMLElement>('[data-revenue-toolbar]')?.getBoundingClientRect().height ?? 148
        ...
        <div ref={tableRef} className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div data-revenue-toolbar className="mb-2 flex flex-col gap-2">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button className='bg-primary text-primary-foreground hover:bg-primary/90' onClick={() => setAddRevenue(true)}>{t('addRevenue')}</Button>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    {range && <>
                        ...
                        <Button disabled={!onRangeChange} size="sm" onClick={() => onRangeChange?.({ from: dateInputToIso(fromInputRef.current?.value || ''), to: dateInputToIso(toInputRef.current?.value || '') })}>{locale === 'en' ? 'Search' : locale === 'zh-TW' ? '搜尋' : 'Tìm'}</Button>
                    </>}
                    <div className="ml-auto flex items-center gap-2">
                        <span className="whitespace-nowrap text-sm text-gray-500">{filteredOrders.length} {t('revenueCount')} • {t('page')} {currentPage}/{totalPages || 1}</span>
