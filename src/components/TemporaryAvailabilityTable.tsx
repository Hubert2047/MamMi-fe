...
export default function TemporaryAvailabilityTable({
  open,
  items,
  addons,
  onClose,
  ...
  const addonMutation = useMutation({
    mutationFn: updateTemporaryStoreAddonAvailability,
    onSuccess: () => {
    ...
    () =>
      addons
        .filter(
          (addon) =>
            !normalizedSearch ||
            addon.name.toLocaleLowerCase().includes(normalizedSearch),
        )
        .sort((a, b) => a.name.localeCompare(b.name)),
    [addons, normalizedSearch],
  );
  ...
  useEffect(() => {
    const element = (tab === "products" ? productsListRef : addonsListRef)
      .current;
    if (!element) return;
    const resize = () => {
      const headerHeight =
      ...
      const nextPageSize = Math.max(
        1,
        Math.floor(
          (availableHeight - headerHeight - 16) / Math.max(rowHeight, 44),
        ) + 3,
      );
      setPageSize((current) =>
    ...
    <TableCell className="text-center">
      <div className="flex justify-center">
        {pendingId === id ? (
          ...
          <Checkbox
            className="size-5 border-2 border-foreground/60 bg-background data-checked:border-primary data-checked:bg-primary"
            checked={unavailable}
            disabled={!permanentlyActive || saving}
            aria-label={`${t("temporaryUnavailable")} ${name}`}
            onCheckedChange={(checked) => {
              setPendingId(id);
              if (kind === "item")
                itemMutation.mutate({
                  itemId: id,
                  temporarilyUnavailable: checked === true,
                });
              else
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
      <DialogContent
        ...
        className="left-0 top-0 flex h-dvh min-h-0 max-h-dvh w-screen max-w-none translate-x-0 translate-y-0 flex-col overflow-hidden rounded-none p-3 pb-[env(safe-area-inset-bottom)] sm:max-w-none sm:px-4"
      >
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 py-1">
            <Input
              className="max-w-sm"
              placeholder={t("temporaryAvailabilitySearch")}
              value={search}
              onChange={(event) => {
            ...
            {tab === "products" && (
              ...
              >
                <SelectTrigger className="w-52">
                  <SelectValue
                    placeholder={t("temporaryAvailabilityCategory")}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t("temporaryAvailabilityAllCategories")}
                  </SelectItem>
          ...
          >
            <div ref={productsListRef} className="min-h-0 flex-1">
              <AvailabilityTable
                items={paginatedItems}
                empty={t("temporaryAvailabilityEmpty")}
                nameHeader={t("name")}
                categoryHeader={t("categories")}
                statusHeader={t("temporaryUnavailable")}
                showCategory
                renderStatus={(item) =>
                  availabilityCell(
                    item._id,
                    item.permanentlyActive !== false,
          ...
          <TabsContent
            ...
            className="flex min-h-0 flex-1 flex-col overflow-hidden"
          >
            <div ref={addonsListRef} className="min-h-0 flex-1">
              <AvailabilityTable
                items={paginatedItems}
                empty={t("temporaryAvailabilityEmpty")}
                nameHeader={t("name")}
                categoryHeader={t("addons")}
                statusHeader={t("temporaryUnavailable")}
                renderStatus={(addon) =>
                  availabilityCell(
                    addon._id,
...
function AvailabilityTable({
  ...
  renderStatus: (item: AvailabilityRow) => ReactNode;
}) {
  return (
    <div className="min-h-0 flex-1 min-w-0 overflow-x-hidden rounded-md border [&>[data-slot=table-container]]:overflow-x-hidden">
        ...
        <TableHeader>
          <TableRow>
            <TableHead
              className={
                showCategory
                  ? "w-[50%] whitespace-normal break-words"
                  : "w-[70%] whitespace-normal break-words"
              }
            >
              {nameHeader}
            </TableHead>
            ...
            <TableHead
              className={
                showCategory
                  ? "w-[15%] whitespace-normal break-words text-center leading-tight"
                  : "w-[30%] whitespace-normal break-words text-center leading-tight"
              }
            >
              {statusHeader}
            </TableHead>
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <TableRow
                key={item._id}
                className={
                  item.permanentlyActive === false ? "opacity-60" : undefined
                }
              >
