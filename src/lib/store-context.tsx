...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const saved =
      typeof window !== "undefined"
        ? window.localStorage.getItem("activeStoreId")
        : null;
    const next = stores.some((store) => store._id === saved)
      ? saved!
      : stores[0]._id;
  ...
  const value = useMemo(
    () => ({
      stores,
      activeStore: stores.find((store) => store._id === activeStoreId) || null,
      activeStoreId,
      setActiveStoreId,
      isLoading,
    }),
    [activeStoreId, isLoading, stores],
  );
  return (
