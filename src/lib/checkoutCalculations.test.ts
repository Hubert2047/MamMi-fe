...
describe('checkout total and cash change', () => {
  it('uses the discounted total when calculating change', () => {
    const total = calculateOrderTotal({
      items: [orderItem],
      appliedPromotions: [{ promotionId: 'ten', promotionVersion: 1, name: '10%', mode: 'manual', discountAmount: 24, allocations: [] }],
    })
  ...
  it('applies a fixed discount before calculating change', () => {
    const total = calculateOrderTotal({
      items: [orderItem],
      appliedPromotions: [{ promotionId: 'fifty', promotionVersion: 1, name: '50 off', mode: 'manual', discountAmount: 50, allocations: [] }],
    })
  ...
  it('does not allow an excessive discount to produce a negative total', () => {
    const total = calculateOrderTotal({
      items: [orderItem],
      appliedPromotions: [{ promotionId: 'free', promotionVersion: 1, name: 'Free', mode: 'manual', discountAmount: 999, allocations: [] }],
    })
  ...
  it('does not return change when the customer has not paid enough', () => {
    const total = calculateOrderTotal({ items: [orderItem], appliedPromotions: [] })
