...
import { calculateActualCash, calculateCashDifference, calculateIncomeTotal, calculateSalesTotal, calculateSystemAmount, requiresClosingReason } from './dailyClosingCalculations'

describe('daily closing calculations', () => {
  it('sums sales from all payment methods and treats missing values as zero', () => {
    expect(calculateSalesTotal({ cash: { totalSales: 120000 }, bank: { totalSales: 80000 }, linepay: undefined })).toBe(200000)
  })
  ...
  it('includes other revenue in the displayed income total', () => {
    expect(calculateIncomeTotal({ cash: { totalSales: 120000 }, bank: { totalSales: 80000 } }, 30000)).toBe(230000)
  })

  it('calculates actual cash from denomination counts', () => {
    expect(calculateActualCash({ '2000': '2', '500': 3, '100': '0' })).toBe(5500)
  })
