...
export default function CategoriesPage() {
  const client = useQueryClient(); const { user } = useAuth(); const { t, locale } = useI18n(); const [names, setNames] = useState<CategoryNames>(emptyNames); const [sortOrder, setSortOrder] = useState(0); const [editing, setEditing] = useState<Category | null>(null)
  const { data: categories = [], isLoading } = useQuery({ queryKey: ['categories'], queryFn: getCategories })
  const save = useMutation({ mutationFn: () => editing ? updateCategory(editing._id, { names, sortOrder }) : createCategory({ names, sortOrder }), onSuccess: () => { client.invalidateQueries({ queryKey: ['categories'] }); client.invalidateQueries({ queryKey: ['admin-items'] }); toast.success(editing ? t('updateCategorySuccess') : t('createCategorySuccess')); setNames(emptyNames); setSortOrder(0); setEditing(null) }, onError: () => toast.error(t('categorySaveError')) })
  ...
  function startEditing(category: Category) { setEditing(category); setNames(category.names); setSortOrder(category.sortOrder ?? 0) }
  function cancelEditing() { setNames(emptyNames); setSortOrder(0); setEditing(null) }
