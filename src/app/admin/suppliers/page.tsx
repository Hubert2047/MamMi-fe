...
export default function SuppliersPage() {
  ...
  const selectableLineGroups = (lineGroups.data ?? []).filter(
    (group: LineGroup) =>
      group.usageStatus === "available" || group._id === form.lineGroupId,
  );
  const totalPages = Math.max(
    1,
    Math.ceil((suppliers.data?.length ?? 0) / pageSize),
  );
  const currentPage = Math.min(page, totalPages);
  const visibleSuppliers = (suppliers.data ?? []).slice(
    ...
    <div className="h-full overflow-hidden p-6 md:p-8">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t(editing ? "supplierEdit" : "supplierAdd")}
            </DialogTitle>
          </DialogHeader>
