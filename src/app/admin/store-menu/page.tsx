...
export default function StoreMenuPage() {
  ...
  const add = useMutation({ mutationFn: addStoreItem, onSuccess: () => { refresh(); setSelectedItemId(''); setPrice(emptyPrice); toast.success(t('createSuccess')) }, onError: () => toast.error(t('saveError')) })
  const update = useMutation({ mutationFn: updateStoreItem, onSuccess: () => { refresh(); setEditing(null); toast.success(t('updateSuccess')) }, onError: () => toast.error(t('saveError')) })
  ...
  const startEdit = (item: Item) => { setEditing(item); setDraftPrice(item.price); setDraftActive(item.active) }
