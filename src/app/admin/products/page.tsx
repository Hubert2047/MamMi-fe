...
export default function ProductsPage() {
  ...
  function itemForm(item: Item): ItemInput {
    ...
    return {
      ...
      addonConfigs: item.addons.map((addon) => ({ addonId: addon._id, maxQuantity: addon.maxQuantity === null ? null : addon.maxQuantity ?? 1 })),
        ...
        {isFormOpen && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-hidden overscroll-contain bg-black/40 p-4 md:p-8"><Card className="flex h-[calc(100svh-2rem)] max-h-[calc(100svh-2rem)] w-full max-w-5xl flex-col overflow-hidden md:h-[calc(100svh-4rem)] md:max-h-[calc(100svh-4rem)]">
          ...
          <CardContent className="min-h-0 overflow-y-auto">
            <form id="product-form" onSubmit={submit} className="space-y-4">
              ...
              {form.type !== 'combo' && <div className="space-y-2"><Label>{t('addons')}</Label><div className="space-y-2 rounded-lg border p-3">{addons.map((addon) => { const selected = form.addons.includes(addon._id); const config = form.addonConfigs.find((entry) => entry.addonId === addon._id); const maxQuantity = config?.maxQuantity === null ? null : config?.maxQuantity ?? 1; const updateConfig = (nextMaxQuantity: number | null) => setForm({ ...form, addonConfigs: form.addonConfigs.map((entry) => entry.addonId === addon._id ? { ...entry, maxQuantity: nextMaxQuantity } : entry) }); return <div key={addon._id} className="flex flex-wrap items-center gap-3 rounded border px-3 py-2"><label className="flex min-w-44 flex-1 items-center gap-2 text-sm"><Checkbox checked={selected} onCheckedChange={(checked) => setForm({ ...form, addons: checked ? [...form.addons, addon._id] : form.addons.filter((id) => id !== addon._id), addonConfigs: checked ? [...form.addonConfigs.filter((entry) => entry.addonId !== addon._id), { addonId: addon._id, maxQuantity: 1 }] : form.addonConfigs.filter((entry) => entry.addonId !== addon._id) })} /><span className="truncate">{addon.names[locale] || addon.names.vi || addon.names.en || addon.names['zh-TW']}</span></label>{selected && <div className="flex items-center gap-2"><label className="flex items-center gap-2 text-sm"><Checkbox checked={maxQuantity === null} onCheckedChange={(checked) => updateConfig(checked === true ? null : 1)} />{t('addonUnlimited')}</label><Label className="text-sm">{t('addonMaxQuantity')}</Label><Input className="h-8 w-20" type="number" min={1} disabled={maxQuantity === null} placeholder={maxQuantity === null ? '∞' : undefined} value={maxQuantity ?? ''} onChange={(event) => updateConfig(Math.max(1, Math.floor(Number(event.target.value) || 1)))} /></div>}</div> })}</div></div>}
