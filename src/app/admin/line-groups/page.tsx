...
const groupStoreId = (group: LineGroup) =>
  typeof group.storeId === "string"
    ? group.storeId
    : ((group.storeId as { _id?: string } | null | undefined)?._id ?? "");
...
export default function LineGroupsPage() {
  ...
  const configuredCount = groups.filter((group) =>
    Boolean(groupStoreId(group)),
  ).length;
        ...
        {visibleGroups.map((group) => (
          <TableRow key={group._id}>
            ...
            <TableCell className="whitespace-nowrap px-2 py-2">
              {stores.find((store) => store._id === groupStoreId(group))
                ?.name || t("lineGroupUnassigned")}
            </TableCell>
            <TableCell className="whitespace-nowrap px-2 py-2">
              {t(
                group.usageStatus === "assigned"
                  ? "lineGroupUsageAssigned"
                  : "lineGroupUsageAvailable",
              )}
            </TableCell>
            <TableCell className="px-2 py-2">
              <div className="flex justify-end gap-2 whitespace-nowrap">
                ...
                <Button
                  ...
                  variant="destructive"
                  disabled={
                    group.usageStatus === "assigned" || remove.isPending
                  }
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden p-6 md:p-8">
      <div className="mb-6 flex shrink-0 items-center justify-between gap-3">
        ...
        >
          <RefreshCw
            className={
              groupsQuery.isFetching ? "size-4 animate-spin" : "size-4"
            }
          />
          {t("lineGroupsRefresh")}
        </Button>
      ...
      >
        <TabsList className="shrink-0">
          ...
          <TabsTrigger value="pending" className="gap-2">
            ...
            <Badge variant={pendingCount ? "destructive" : "secondary"}>
              {pendingCount}
            </Badge>
          </TabsTrigger>
        </TabsList>
            ...
            <CardContent className="min-h-0">
              <div className="h-full overflow-hidden">
                {groupsQuery.isLoading ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    {tab === "pending"
                      ? t("lineGroupPendingEmpty")
                      : t("lineGroupConfiguredEmpty")}
                  </p>
                )}
              </div>
            </CardContent>
