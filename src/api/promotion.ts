...
const orderItemTotal = (item: Pick<OrderItem, 'basePrice' | 'quantity' | 'addons'>) => item.basePrice * item.quantity + item.addons.reduce((total, addon) => total + addon.amount * addon.priceExtra * item.quantity, 0)
const promotionDiscount = (subtotal: number, reward: PromotionRule['reward']) => Math.max(0, Math.min(subtotal, reward.type === 'percent' ? subtotal * reward.amount / 100 : reward.amount))

/** Fast POS-only preview. The backend recalculates and validates this snapshot before accepting an order. */
export const calculatePromotionPreview = ({ items, promotions, selectedPromotionIds = [], now = new Date() }: { items: OrderItem[]; promotions: Promotion[]; selectedPromotionIds?: string[]; now?: Date }): { total: number; appliedPromotions: AppliedPromotion[] } => {
    const grossSubtotal = items.reduce((total, item) => total + orderItemTotal(item), 0)
    const eligible = promotions.filter((promotion) => {
        if (!promotion.enabled || promotion.status !== 'active') return false
        if (promotion.mode !== 'automatic' && !selectedPromotionIds.includes(promotion._id)) return false
        if (promotion.minSubtotal && grossSubtotal < promotion.minSubtotal) return false
        return (!promotion.startsAt || new Date(promotion.startsAt) <= now) && (!promotion.endsAt || new Date(promotion.endsAt) >= now)
    })
    const estimatedDiscount = (promotion: Promotion) => promotion.rules.reduce((sum, rule) => {
        if (rule.target === 'order') return sum + promotionDiscount(grossSubtotal, rule.reward)
        return sum + items.reduce((itemSum, item) => {
            if (rule.productIds?.length && !rule.productIds.includes(item.id)) return itemSum
            if (rule.target === 'product') return itemSum + promotionDiscount(item.basePrice * item.quantity, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
            if (rule.target === 'addon') return itemSum + item.addons.filter((addon) => !rule.addonIds?.length || rule.addonIds.includes(addon.id)).reduce((addonSum, addon) => addonSum + promotionDiscount(addon.amount * addon.priceExtra * item.quantity, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * addon.amount * item.quantity : rule.reward.amount }), 0)
            return itemSum + promotionDiscount(orderItemTotal(item), { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
        }, 0)
    }, 0)
    const sorted = [...eligible].sort((a, b) => b.priority - a.priority || estimatedDiscount(b) - estimatedDiscount(a) || a._id.localeCompare(b._id))
    ...
    for (const promotion of [...accepted.filter((entry) => entry.rules.some((rule) => rule.target !== 'order')), ...accepted.filter((entry) => entry.rules.every((rule) => rule.target === 'order'))]) {
        ...
        for (const rule of promotion.rules) {
            ...
            items.forEach((item, itemIndex) => {
                if (rule.productIds?.length && !rule.productIds.includes(item.id)) return
                if (rule.target === 'product') {
                    const discount = promotionDiscount(remainingProduct[itemIndex]!, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
                    remainingProduct[itemIndex]! -= discount
                    allocations[itemIndex]!.productDiscountAmount += discount
                } else if (rule.target === 'line') {
                    let remainingDiscount = promotionDiscount(remainingProduct[itemIndex]! + remainingAddon[itemIndex]!.reduce((sum, amount) => sum + amount, 0), { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
                    ...
                    item.addons.forEach((addon, addonIndex) => {
                        const discount = Math.min(remainingAddon[itemIndex]![addonIndex]!, remainingDiscount)
                        remainingAddon[itemIndex]![addonIndex]! -= discount
                        remainingDiscount -= discount
                        if (discount) allocations[itemIndex]!.addonDiscounts.push({ addonId: addon.id, discountAmount: discount })
                    })
                } else item.addons.forEach((addon, addonIndex) => {
                    if (rule.addonIds?.length && !rule.addonIds.includes(addon.id)) return
                    const discount = promotionDiscount(remainingAddon[itemIndex]![addonIndex]!, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity * addon.amount : rule.reward.amount })
                    remainingAddon[itemIndex]![addonIndex]! -= discount
                    if (discount) allocations[itemIndex]!.addonDiscounts.push({ addonId: addon.id, discountAmount: discount })
                })
            })
        ...
        if (discountAmount) appliedPromotions.push({ promotionId: promotion._id, promotionVersion: promotion.version, name: promotion.name, mode: promotion.mode, targets: [...new Set(promotion.rules.map((rule) => rule.target))], discountAmount, allocations })
    }
    return { total: remainingProduct.reduce((sum, amount) => sum + amount, 0) + remainingAddon.reduce((sum, addons) => sum + addons.reduce((addonSum, amount) => addonSum + amount, 0), 0), appliedPromotions }
}

const isAvailableForCatalog = (promotion: Promotion, now: Date, includeConditional = false) => {
    if (!promotion.enabled || promotion.status !== 'active' || promotion.mode !== 'automatic' || (!includeConditional && promotion.minSubtotal)) return false
    return (!promotion.startsAt || new Date(promotion.startsAt) <= now) && (!promotion.endsAt || new Date(promotion.endsAt) >= now)
}
...
const eligibleCatalogPromotions = (promotions: Promotion[], now: Date, matchesRule: (rule: PromotionRule) => boolean, includeConditional = false) => {
    const candidates = promotions.filter((promotion) => isAvailableForCatalog(promotion, now, includeConditional) && promotion.rules.some(matchesRule))
 ...
 */
export const getCatalogProductPromotionPrice = ({ productId, price, promotions, now = new Date(), includeConditional = false }: { productId: string; price: number; promotions: Promotion[]; now?: Date; includeConditional?: boolean }) => {
    const matchesRule = (rule: PromotionRule) => rule.target === 'product' && (!rule.productIds?.length || rule.productIds.includes(productId))
    const accepted = eligibleCatalogPromotions(promotions, now, matchesRule, includeConditional)
