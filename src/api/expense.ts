...
export const getExpenseUnits = async (
...
export const createExpenseUnit = async (
  data: Omit<ExpenseUnit, "_id" | "active"> & { active?: boolean },
): Promise<ExpenseUnit> => (await api.post("units", data)).data.data;
export const updateExpenseUnit = async ({
  id,
  data,
}: {
  id: string;
  data: Partial<ExpenseUnit>;
}): Promise<ExpenseUnit> => (await api.put(`units/${id}`, data)).data.data;
