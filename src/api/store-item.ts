...
export const getStoreItems = async (lang?: string): Promise<Item[]> =>
  (await api.get("store-items", { params: lang ? { lang } : {} })).data.data;
export const addStoreItem = async (data: {
  itemId: string;
  price: PriceType;
}) => (await api.post("store-items", data)).data.data;
export type StoreItemVisibility = {
  pos: boolean;
  qr: boolean;
  online: boolean;
};
...
export const updateTemporaryStoreItemAvailability = async ({
  ...
  (
    await api.patch(`store-items/${itemId}/temporary-availability`, {
      temporarilyUnavailable,
    })
  ).data.data;
