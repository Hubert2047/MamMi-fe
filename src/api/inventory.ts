...
export const updateInventoryReceipt = async ({ id, data }: { id: string; data: InventoryReceiptInput }): Promise<unknown> => (await api.put(`inventory/receipts/${id}`, data)).data.data
export const deleteInventoryReceipt = async (id: string): Promise<unknown> => (await api.delete(`inventory/receipts/${id}`)).data.data
